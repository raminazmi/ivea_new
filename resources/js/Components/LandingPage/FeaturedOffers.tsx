...
interface Offer {
    ...
    image_path?: string;
    featured_offers_setting_id?: number;
    featured_offers_setting?: FeaturedOffersSetting;
}
...
const FeaturedOffers: React.FC<FeaturedOffersProps> = ({ offers = [], offersTexts = [], featuredOffersSettings }) => {
    ...
    const getOffersText = (key: string, type: 'title' | 'description' = 'title') => {
        if (!Array.isArray(offersTexts)) return '';
        
        const text = offersTexts.find(t => t.key === key);
        if (!text) return '';
        
        if (type === 'title') {
            return text.title_ar || '';
        } else if (type === 'description') {
            return text.description_ar || '';
        }
        
        return '';
    };

    const getCategoryImage = (offer: Offer) => {
        if (offer.image_path) {
    ...
    const filteredOffers = featuredOffersSettings 
        ? offers.filter(offer => offer.featured_offers_setting_id === featuredOffersSettings.id)
        : offers;
                ...
                <div className="flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-8 lg:gap-12">
                    <div ref={contentRef} className="text-center lg:text-start w-full lg:w-1/2 mb-6 lg:mb-0">
                        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#0D1D25] mb-2 md:mb-3">
                            {featuredOffersSettings?.title_ar || getOffersText('main_title', 'title') || 'عــــرض اليوم الوطـنـي'}
                        </h2>
                        <p className="text-xs sm:text-sm md:text-base text-[#424242] max-w-2xl mx-auto lg:mx-0">
                            {featuredOffersSettings?.description_ar || getOffersText('main_title', 'description') || 'اطلع على عروضنا اليومية التي تجمع بين خصومات كبيرة على منتجات المنزل. العروض قد تتغير يوميا، فلا تفوت الفرصة :)'}
                        </p>
                        <div className='mt-4 flex justify-center lg:justify-start'>
                            <Link
                                href={featuredOffersSettings?.button_url || "/products"}
                            ...
                            >
                                {featuredOffersSettings?.button_text_ar || "افتح المتجر"}
                            </Link>
                        </div>
                    </div>
